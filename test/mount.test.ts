...
describe("mountUI", () => {
  test("injects exactly one host with an ez button and a panel container", () => {
    ...
    expect(shadow().querySelector(".abm-ez-button")!.textContent).toBe("ez");
    expect(shadow().querySelector(".abm-container")).not.toBeNull();
  });
  ...
  test("clicking the ez button toggles the panel open state", () => {
    mountUI(storage);
    const container = shadow().querySelector(".abm-container")!;
    const button = shadow().querySelector<HTMLButtonElement>(".abm-ez-button")!;
    expect(container.classList.contains("abm-open")).toBe(false);
    button.click();
    expect(container.classList.contains("abm-open")).toBe(true);
    button.click();
    expect(container.classList.contains("abm-open")).toBe(false);
  });
  ...
  test("toggling the master switch persists to storage", () => {
    mountUI(storage);
    const master = shadow().querySelector<HTMLInputElement>(".abm-master")!;
  ...
  test("adding a rule persists a new rule and re-renders a card for it", () => {
    mountUI(storage);
    shadow().querySelector<HTMLButtonElement>(".abm-add")!.click();
    expect(loadConfig(storage).rules.length).toBe(DEFAULT_CONFIG.rules.length + 1);
    expect(shadow().querySelectorAll(".abm-rule").length).toBe(DEFAULT_CONFIG.rules.length + 1);
  });

  test("editing a rule's outcome persists via slots", () => {
    mountUI(storage);
    const sel = shadow().querySelector<HTMLSelectElement>(".abm-rule .abm-outcome")!;
    sel.value = "loss";
